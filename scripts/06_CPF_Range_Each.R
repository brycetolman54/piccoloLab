...
for(dataSet in dataSetVariables) {
   ...
   eval(expr(!!sym(dataSets[i]) <<- replace(!!sym(dataSets[i]), is.na(!!sym(dataSets[i])), 0)))
...
saveRDS(fitModel, "models/06_CPF_Range_Each.rds")
...
for(dataSet in dataSetVariables) {
    ...
    rocCurve(fitModel,
             ...
             mdText = "CPF Range Standardization Each",
